...
- [x] Implement Promise-returning `page.goto`, `page.prepare`, `page.preflight`, `page.inspect`, `page.inspectAll`, and `page.close`.
- [x] Implement lowerCamel JS option/result codecs with validation errors.
- [x] Implement JS-visible error classes/codes: `CvdError`, `SelectorError`, `PrepareError`, `BrowserError`, `ArtifactError`.
...
- [x] Run `go test ./internal/cssvisualdiff/dsl ./internal/cssvisualdiff/verbcli ./cmd/css-visual-diff`.
- [x] Commit Phase 4 Promise-first JS module.
...
- [x] Add built-in `catalog inspect-config` or YAML interop verb.
