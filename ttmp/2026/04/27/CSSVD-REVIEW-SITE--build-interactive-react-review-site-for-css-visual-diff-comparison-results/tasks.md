...
- [x] Create TypeScript types: summary.ts, compare.ts, review.ts
- [x] Create RTK store: cardsSlice, viewSlice, reviewSlice, commentsSlice, localStorage middleware
- [x] Build utility functions: paths.ts (artifact URL rewriting), export.ts (markdown+yaml), storage.ts
- [x] Build layout components: App.tsx, Header.tsx, FilterToolbar.tsx, CardList.tsx
- [x] Build view mode components: ViewModeSideBySide, ViewModeOverlay, ViewModeSlider, ViewModeDiff
- [x] Build ReviewCard + ImageCanvas + CommentPin components
- [x] Build Sidebar with CommentsTab, StylesTab, MetaTab
- [x] Build ExportModal component
- [x] Create Go internal/web package: embed.go, embed_none.go, static.go, generate.go
- [x] Create cmd/build-web Dagger pipeline (with local fallback)
