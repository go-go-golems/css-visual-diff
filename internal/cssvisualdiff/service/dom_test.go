...
setTimeout(() => {
  document.getElementById('hidden').style.display = 'block';
	...
	visible, err := WaitForLocator(page, LocatorSpec{Selector: "#hidden"}, WaitForSelectorOptions{TimeoutMS: 2000, PollIntervalMS: 50})
	require.NoError(t, err)
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "did not become visible")
...
func TestWaitForLocatorAllowsHiddenWhenVisibleFalse(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	present, err := WaitForLocator(page, LocatorSpec{Selector: "#hidden"}, WaitForSelectorOptions{TimeoutMS: 500, PollIntervalMS: 25, Visible: &visibleFalse})
	require.NoError(t, err)
	require.True(t, present.Exists)
	require.False(t, present.Visible)
	require.Equal(t, "#hidden", present.Selector)

	notVisible, err := WaitForLocator(page, LocatorSpec{Selector: "#hidden"}, WaitForSelectorOptions{TimeoutMS: 100, PollIntervalMS: 25})
	require.Error(t, err)
	require.Contains(t, err.Error(), "did not become visible")
	require.True(t, notVisible.Exists)
	require.False(t, notVisible.Visible)
}
